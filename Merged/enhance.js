...
const apiurl = `https://shit-taste.net/api/commentfacescall.php?sub=${url}`;
const commentfaces = [];
...
function onGot(result) {
  if (result.opcommentfaces !== undefined) { cf = result.opcommentfaces; } else { cf = true; }

  if (result.opanisearch !== undefined) { as = result.opanisearch; } else { as = true; }

  if (result.opspoiler !== undefined) { sp = result.opspoiler; } else { sp = false; }

  if (result.nuspoiler !== undefined) { nsp = result.nuspoiler; } else { nsp = false; }
...
function createMenu(cf, as, sp) {
  ...
  if (cf) { menu += '<a class="stab showcommentfacestab">Commentfaces</a>'; }
  if (as) { menu += '<a class="stab anilisttab">AniList</a>'; }
  if (sp) { menu += '<a class="stab formattab">Formatting</a>'; }
  if (cf) {
...
function createCommentfaces(innerform) {
  ...
  */
  ...
  function generateCommentfaces(filteredFace, texttop, textbottom, texthover) {
    ...
    } if (url == 'anime' || url == 'ftfanime' || url == 'AnimeImpressions') {
  ...
  showrecentcommentfaces.click(function () {
    ...
    if (storageItemList !== null) {
      ...
      storageItemList.forEach((itemList) => {
        ...
        if (commentfaces.includes(itemList.replace('#', ''))) { result += generateCommentfaces(itemList.replace('#', ''), texttop, textbottom, texthover); }
      });
    }
...
function searchOnAniList(searchterm, targetelement, field) {
  ...
  function handleData(data) {
    ...
    result += '</table>';
    if (result === '<table class="browseAniListtable"></table>') { result = 'No records to your query returned.'; }
